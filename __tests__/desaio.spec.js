...
it('desafioRecusado', async () => {
	...
	await expect(context.sendText).toBeCalledWith(flow.desafioRecusado.text2, await checkAnsweredQuiz(context, opt.greetings));
});
...
it('asksDesafio - user finished quiz ', async () => {
	...
	await expect(context.sendText).toBeCalledWith(flow.desafioRecusado.text2, await checkAnsweredQuiz(context, options));
});
